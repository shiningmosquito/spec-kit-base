...
# Fetch version config dynamically from package.json if node is available
if command -v node >/dev/null 2>&1; then
    HUGO_VERSION=$(node -p "require('./package.json').config?.hugo || '0.163.3'")
    GO_VERSION=$(node -p "require('./package.json').config?.go || '1.22.4'")
else
